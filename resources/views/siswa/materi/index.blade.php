            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow flex flex-col justify-between border-l-4 border-blue-500">
                    <div>
                        ...
                        <h3 class="mt-2 text-xl font-bold text-gray-900 dark:text-white">{{ $m->judul }}</h3>
                        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Oleh: {{ $m->mapel->guru->name }} | {{ $m->tanggal_upload }}</p>
                    </div>
                    <div class="mt-4">
                        <a href="{{ route('siswa.materi.view', $m->id) }}" class="inline-block px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Lihat Materi</a>
                    </div>
                </div>
                @empty
